 ...
 *
 * Usage:
 *   node run-integration-test.js                           # Local tests only
 *   node run-integration-test.js --interactive             # Prompt for server details
 *   node run-integration-test.js --server=... --token=...  # Direct configuration
 */
...
function parseArgs() {
  ...
  for (const arg of args) {
    if (arg === '--interactive' || arg === '-i') {
      ...
      console.log(`
Usage: node run-integration-test.js [options]
...
Examples:
  ...
  node run-integration-test.js --interactive

  # Direct configuration
  node run-integration-test.js --server=https://localhost:4332 --token=abc123...
`);
      process.exit(0);
    }
  }
